 ...
 */
function wrtCalendarByBtn(ojdd, ojyyyy, oymm, btnId) {
//    wrtCalendar(ojdd, ojyyyy, oymm ,'today', false, btnId);
    wrtCalendar(ojdd, ojyyyy, oymm , '', false, btnId);
}
...
function wrtCalendarWithReload(oj, objmm, objyyyy, arg1, arg2, btnId ) {
    reloadFlg = true;
    wrtCalendar(oj, objmm, objyyyy, arg1, arg2, btnId );
}
 ...
 * @param btnId カレンダーボタンのid
 */
function wrtCalendar(oj, objmm, objyyyy, arg1, arg2, btnId ) {
    ...
    if (arg1==0) {
       ...
       if(today || paramvalue==""){
          now = new Date();
           ...
           //yyyy/mm/dd文字列を作成
           var ymdf = escape(objyyyy.value + '/' + (objmm.value) + "/" + oj.value);
           //RegExpオブジェクトの作成
           re = new RegExp(/(\d{4})\/(\d{1,2})\/(\d{1,2})/);
           if (ymdf.match(re)) {
               now   = new Date(objyyyy.value ,objmm.value -1,oj.value)
           } else {
               /*
           ...
           var yyyy = escape(objyyyy.value);
           var mm = escape(objmm.value);
           var dd = escape(oj.value);
           alert (yyyy + ":" + mm + ":" + dd);
...
//  }
  ...
  //-MONTH
    ...
    ddata += '<a href="javascript:self.opener.wrtCalendar(self.opener.document.'+oj.form.name+'.'+oj.name + ',self.opener.document.' + oj.form.name + '.' + objmm.name + ',self.opener.document.' + oj.form.name + '.' + objyyyy.name +' ,' + (nowyear-1) + nowmonth +',0);"><span class="small">&lt;</span></a>\n'

    ddata += '<a href="javascript:self.opener.wrtCalendar(self.opener.document.'+oj.form.name+'.'+oj.name + ',self.opener.document.' + oj.form.name + '.' + objmm.name + ',self.opener.document.' + oj.form.name + '.' + objyyyy.name +' ,' + (nowyear-1) + nowmonth +',0);"><span class="small">' + msglist.prevYear +'</span></a>&nbsp;&nbsp;&nbsp;&nbsp;\n'

    ddata += '   <select name="selectyear" onchange="self.opener.wrtCalendar(self.opener.document.'+oj.form.name+'.'+oj.name + ',self.opener.document.' + oj.form.name + '.' + objmm.name + ',self.opener.document.' + oj.form.name + '.' + objyyyy.name +' , document.forms[0].selectyear.value , 0)">\n>\n'
    ...
    ddata += '   <select name="selectmonth" onchange="self.opener.wrtCalendar(self.opener.document.'+oj.form.name+'.'+oj.name + ',self.opener.document.' + oj.form.name + '.' + objmm.name + ',self.opener.document.' + oj.form.name + '.' + objyyyy.name +' , document.forms[0].selectmonth.value , 0)">\n';
    ...
    ddata += '&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:self.opener.wrtCalendar(self.opener.document.'+oj.form.name+'.'+oj.name + ',self.opener.document.' + oj.form.name + '.' + objmm.name + ',self.opener.document.' + oj.form.name + '.' + objyyyy.name +' , ' + (nowyear+1) + nowmonth + ',0);"><span class="small">' + msglist.nextYear + '</span></a>\n'
    ...
    ddata += '          onClick="self.opener.wrtCalendar(self.opener.document.'+oj.form.name+'.'+oj.name + ',self.opener.document.' + oj.form.name + '.' + objmm.name + ',self.opener.document.' + oj.form.name + '.' + objyyyy.name +' ,' + nowyear + beforemonth + ',0)"\n'
    ddata += '   > \n'
    ddata += '   </td>\n'

    ddata += '   <td align="center" width="100%">\n'
    ddata += '   <input class="now_btn" type="button" VALUE="' + msglist.thisMonth + '" \n'
    ddata += 'onClick="self.opener.wrtCalendar(self.opener.document.'+oj.form.name+'.'+oj.name+',self.opener.document.' + oj.form.name + '.' + objmm.name + ',self.opener.document.' + oj.form.name + '.' + objyyyy.name + ',\'today\',0)"\n'
    ddata += '>'
    ddata += '   </td>\n'
    ...
    ddata += 'onClick="self.opener.wrtCalendar(self.opener.document.'+oj.form.name+'.'+oj.name + ',self.opener.document.' + oj.form.name + '.' + objmm.name + ',self.opener.document.' + oj.form.name + '.' + objyyyy.name +' ,' + nowyear + nextmonth + ',0)"\n'
    ddata += '>'
    ddata += '   </td>\n'
  ...
  　    //日TR
    ...
    for(i=0;i<7;i++){
      ...
      wrtdateA += '.'+oj.name+'.value=(\''+wrtdate2+'\');'
      //月
      wrtdateA += '   self.opener.document.'+oj.form.name
      wrtdateA += '.'+objmm.name+'.value=(\''+ (wrtmonth + 1)+'\');'
      //年
      wrtdateA += '   self.opener.document.'+oj.form.name
      wrtdateA += '.'+objyyyy.name+'.value=(\''+wrtyear+'\');'

      if (calDateClickFnc) {
          wrtdateA += '   window.opener.' + calDateClickFnc + ';';
      }

      if (!reloadFlg) {
          wrtdateA += 'self.close()};v()" '
      } else {
          wrtdateA += 'window.opener.document.forms[0].submit();self.close()};v()" '
      }
...
//  }
    ddata += '   <TR>\n'
    ddata += '   <TD COLSPAN="7" ALIGN="center">\n'
    ddata += '   <input type="button" class="close_btn2" value="' + msglist.close + '" '
    ddata += '          onClick="self.close();return false">\n'
    ddata += '   </TD>\n'
    ddata += '   </TR>\n'
