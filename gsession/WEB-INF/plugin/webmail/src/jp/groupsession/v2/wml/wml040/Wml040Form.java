...
/**
 * <br>[機  能] WEBメール アカウント登録画面のフォーム
 * <br>[解  説]
 * <br>[備  考]
 *
 * @author JTS
 */
public class Wml040Form extends Wml020Form {
     ...
     * @return エラー
     */
    public ActionErrors validateCheck(Connection con, RequestModel reqMdl) throws Exception {
        ActionErrors errors = new ActionErrors();
        GsMessage gsMsg = new GsMessage(reqMdl);

        //アカウント名入力チェック
        GSValidateWebmail.validateTextBoxInput(errors, wml040name__,
                "wml040name",
                gsMsg.getMessage(GSConstWebmail.TEXT_ACCOUNT),
                GSConstWebmail.MAXLEN_ACCOUNT, true);
        if (errors.isEmpty()) {
            //重複チェック
            WmlAccountDao accountDao = new WmlAccountDao(con);
            if (accountDao.existAccount(wml040name__, getWmlAccountSid())) {
                ActionMessage msg = new ActionMessage("error.select.dup.list",
                                    gsMsg.getMessage(GSConstWebmail.TEXT_ACCOUNT));
                StrutsUtil.addMessage(errors, msg, "wml040name." + "error.select.dup.list");
            }
        }

        //メールアドレス入力チェック
        GSValidateWebmail.validateMailWithNameTextBoxInput(errors, wml040mailAddress__,
                "wml040mailAddress",
                gsMsg.getMessage(GSConstWebmail.TEXT_ADDRESS),
                GSConstWebmail.MAXLEN_ACCOUNT_ADDRESS, true);

        //メール受信サーバー名入力チェック
        GSValidateWebmail.validateTextBoxInput(errors, wml040receiveServer__,
                "wml040receiveServer",
                gsMsg.getMessage(GSConstWebmail.TEXT_SERVER_RECEIVE),
                GSConstWebmail.MAXLEN_ACCOUNT_SERVER, true);

        //メール受信サーバー名ポート番号入力チェック
        GSValidateWebmail.validateNumber(errors, wml040receiveServerPort__,
                "wml040mailPort1",
                gsMsg.getMessage(GSConstWebmail.TEXT_PORT_RECEIVE),
                GSConstWebmail.MAXLEN_ACCOUNT_PORT);

        //メール受信サーバーユーザ名入力チェック
        GSValidateWebmail.validateTextBoxInput(errors, wml040receiveServerUser__,
                "wml040receiveServerUser",
                gsMsg.getMessage(GSConstWebmail.TEXT_USER_RECEIVE),
                GSConstWebmail.MAXLEN_ACCOUNT_SERVER_USER, true);

        //メール受信サーバーパスワード入力チェック
        GSValidateWebmail.validateTextBoxInput(errors, wml040receiveServerPassword__,
                "wml040receiveServerPassword",
                gsMsg.getMessage(GSConstWebmail.TEXT_PASS_RECEIVE),
                GSConstWebmail.MAXLEN_ACCOUNT_SERVER_PASS, true);

        //メール送信サーバ名入力チェック
        GSValidateWebmail.validateTextBoxInput(errors, wml040sendServer__,
                "wml040sendServer",
                gsMsg.getMessage(GSConstWebmail.TEXT_SERVER_SEND),
                GSConstWebmail.MAXLEN_ACCOUNT_SERVER, true);

        //メール送信サーバユーザ名入力チェック
        GSValidateWebmail.validateTextBoxInput(errors, wml040sendServerUser__,
                "wml040sendServerUser",
                gsMsg.getMessage(GSConstWebmail.TEXT_USER_SEND),
                GSConstWebmail.MAXLEN_ACCOUNT_SERVER_USER, false);

        //メール送信サーバ名　ポート番号
        GSValidateWebmail.validateNumber(errors, wml040sendServerPort__,
                "wml040mailPort2",
                gsMsg.getMessage(GSConstWebmail.TEXT_PORT_SEND),
                GSConstWebmail.MAXLEN_ACCOUNT_PORT);

        //メール送信サーバパスワード入力チェック
        GSValidateWebmail.validateTextBoxInput(errors, wml040sendServerPassword__,
                "wml040sendServerPassword",
                gsMsg.getMessage(GSConstWebmail.TEXT_PASS_SEND),
                GSConstWebmail.MAXLEN_ACCOUNT_SERVER_PASS, false);

        //ディスク容量最大値入力チェック
        //制限ありのときエラーチェック
        if (wml040diskSize__ == 1) {
            GSValidateWebmail.validateNumber(errors, wml040diskSizeLimit__,
                    "wml040diskSize",
        ...
        //備考入力チェック
        GSValidateWebmail.validateTextarea(errors, wml040biko__,
                "wml040biko",
                gsMsg.getMessage(GSConstWebmail.TEXT_BIKO),
                GSConstWebmail.MAXLEN_ACCOUNT_BIKO,
                false);

        if (getWmlAccountMode() == GSConstWebmail.ACCOUNTMODE_COMMON) {
            //使用者 グループ 設定チェック
            if (wml040userKbn__ == GSConstWebmail.USRKBN_GROUP) {
                if (wml040userKbnGroup__ == null) {
            ...
            //使用者 ユーザ 設定チェック
            } else if (wml040userKbn__ == GSConstWebmail.USRKBN_USER) {
                if (wml040userKbnUser__ == null) {
        ...
        //表示区分通常設定のみ表示の場合のエラーチェック
        //組織名入力チェック
        GSValidateWebmail.validateTextBoxInput(errors, wml040organization__,
                "wml040organization",
                gsMsg.getMessage(GSConstWebmail.TEXT_ORGANIZE),
                GSConstWebmail.MAXLEN_ACCOUNT_ORGANIZE, false);

        //署名入力チェック
        GSValidateWebmail.validateTextarea(errors, wml040sign__,
                "wml040sign",
        ...
        //自動TO入力チェック
        GSValidateWebmail.validateMailTextBoxInput(errors, wml040autoTo__,
                "wml040autoTo",
                gsMsg.getMessage(GSConstWebmail.TEXT_AUTO_TO),
                GSConstWebmail.MAXLEN_ACCOUNT_ADDRESS, false);

        //自動CC入力チェック
        GSValidateWebmail.validateMailTextBoxInput(errors, wml040autoCc__,
                "wml040autoCc",
                gsMsg.getMessage(GSConstWebmail.TEXT_AUTO_CC),
                GSConstWebmail.MAXLEN_ACCOUNT_ADDRESS, false);

        //自動BCC入力チェック
        GSValidateWebmail.validateMailTextBoxInput(errors, wml040autoBcc__,
                "wml040autoBcc",
                gsMsg.getMessage(GSConstWebmail.TEXT_AUTO_BCC),
                GSConstWebmail.MAXLEN_ACCOUNT_ADDRESS, false);
    ...
    public void setHiddenParam(Cmn999Form msgForm) {
        ...
        msgForm.addHiddenParam("wml040name", getWml040name());
        msgForm.addHiddenParam("wml040mailAddress", getWml040mailAddress());
        msgForm.addHiddenParam("wml040receiveServer", getWml040receiveServer());
        msgForm.addHiddenParam("wml040receiveServerPort", getWml040receiveServerPort());
        msgForm.addHiddenParam("wml040receiveServerSsl", getWml040receiveServerSsl());
        msgForm.addHiddenParam("wml040receiveServerType", getWml040receiveServerType());
        msgForm.addHiddenParam("wml040receiveServerUser", getWml040receiveServerUser());
        msgForm.addHiddenParam("wml040receiveServerPassword", getWml040receiveServerPassword());
        msgForm.addHiddenParam("wml040sendServer", getWml040sendServer());
        msgForm.addHiddenParam("wml040sendServerPort", getWml040sendServerPort());
        msgForm.addHiddenParam("wml040sendServerSsl", getWml040sendServerSsl());
        msgForm.addHiddenParam("wml040sendServerUser", getWml040sendServerUser());
        msgForm.addHiddenParam("wml040sendServerPassword", getWml040sendServerPassword());
        msgForm.addHiddenParam("wml040diskSize", getWml040diskSize());
        msgForm.addHiddenParam("wml040diskSizeLimit", getWml040diskSizeLimit());
        msgForm.addHiddenParam("wml040biko", getWml040biko());
        msgForm.addHiddenParam("wml040userKbn", getWml040userKbn());
        msgForm.addHiddenParam("wml040userKbnGroup", getWml040userKbnGroup());
        msgForm.addHiddenParam("wml040userKbnUser", getWml040userKbnUser());
        msgForm.addHiddenParam("wml040organization", getWml040organization());
        msgForm.addHiddenParam("wml040sign", getWml040sign());
        msgForm.addHiddenParam("wml040autoTo", getWml040autoTo());
        msgForm.addHiddenParam("wml040autoCc", getWml040autoCc());
        msgForm.addHiddenParam("wml040autoBcc", getWml040autoBcc());
        msgForm.addHiddenParam("wml040delReceive", getWml040delReceive());
        msgForm.addHiddenParam("wml040reReceive", getWml040reReceive());
        msgForm.addHiddenParam("wml040apop", getWml040apop());
        msgForm.addHiddenParam("wml040smtpAuth", getWml040smtpAuth());
        msgForm.addHiddenParam("wml040popBSmtp", getWml040popBSmtp());
        msgForm.addHiddenParam("wml040encodeSend", getWml040encodeSend());

        msgForm.addHiddenParam("wml040initFlg", getWml040initFlg());
        msgForm.addHiddenParam("wml040userKbnGroupSelect", getWml040userKbnGroupSelect());
        msgForm.addHiddenParam("wml040userKbnGroupNoSelect", getWml040userKbnGroupNoSelect());
        msgForm.addHiddenParam("wml040userKbnUserSelect", getWml040userKbnUserSelect());
        msgForm.addHiddenParam("wml040userKbnUserNoSelect", getWml040userKbnUserNoSelect());
        msgForm.addHiddenParam("wml040userKbnUserGroup", getWml040userKbnUserGroup());
        msgForm.addHiddenParam("wml040elementKbn", getWml040elementKbn());
        ...
        if (getWmlAccountMode() == GSConstWebmail.ACCOUNTMODE_COMMON) {
            ...
            msgForm.addHiddenParam("wml100sortAccount", getWml100sortAccount());
        }
    }
