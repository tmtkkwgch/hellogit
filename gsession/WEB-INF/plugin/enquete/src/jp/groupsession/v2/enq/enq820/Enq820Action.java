...
/**
 * <br>[機  能] アンケート 個人設定 メイン表示設定画面のアクションクラス
 * <br>[解  説]
 * <br>[備  考]
 *
 * @author JTS
 */
public class Enq820Action extends AbstractEnqueteAction {

    /** Logging インスタンス */
    private static Log log__ = LogFactory.getLog(Enq820Action.class);
    ...
    public ActionForward executeAction(ActionMapping map,
        ...
        log__.debug("Enq820Action_START");

        ActionForward forward = null;
        Enq820Form enq820Form = (Enq820Form) form;

        // アクセス権限チェック
        con.setAutoCommit(true);
        try {
            EnqCommonBiz enqBiz = new EnqCommonBiz();
            if (!enqBiz.checkPriPerm(getRequestModel(req), con, GSConstEnquete.DSP_ID_820)) {
        ...
        // コマンドの判定
        if (cmd.equals("enq820commit")) {
            // 更新処理
            forward = __doCommit(map, enq820Form, req, res, con);

        } else if (cmd.equals("enq820back")) {
            // 戻る
            forward = map.findForward(cmd);

        } else {
            //初期表示処理
            forward = __doInit(map, enq820Form, req, res, con);
        }

        log__.debug("Enq820Action_END");
    ...
    throws Exception {
        ...
        con.setAutoCommit(true);
        try {
            Enq820Biz biz = new Enq820Biz();
            Enq820ParamModel paramModel = new Enq820ParamModel();
    ...
    private ActionForward __setCommitDsp(ActionMapping map,
        ...
        // OKボタンクリック時遷移先
        urlForward = map.findForward("enq820commit");
        cmn999Form.setUrlOK(urlForward.getPath());

        // メッセージのセット
        GsMessage gsMsg = new GsMessage();
        String textEnq = gsMsg.getMessage(req, "enq.enq800.03");
