...
/**
 * <br>[機  能] 管理者設定 メイン表示設定画面のアクションクラス
 * <br>[解  説]
 * <br>[備  考]
 *
 * @author JTS
 */
public class Enq940Action extends AbstractEnqueteAction {

    /** Logging インスタンス */
    private static Log log__ = LogFactory.getLog(Enq940Action.class);
    ...
    public ActionForward executeAction(ActionMapping map,
        ...
        log__.debug("Enq940Action_START");

        ActionForward forward = null;
        Enq940Form enq940Form = (Enq940Form) form;
        ...
        // コマンドの判定
        if (cmd.equals("enq940commit")) {
            // 登録
            forward = __doCommit(map, enq940Form, req, res, con);

        } else if (cmd.equals("enq940back")) {
            // 戻る
            forward = map.findForward(cmd);

        } else {
            //初期表示処理
            forward = __doInit(map, enq940Form, req, res, con);
        }

        log__.debug("Enq940Action_END");
    ...
    throws Exception {
        ...
        try {
            Enq940Biz biz = new Enq940Biz();
            Enq940ParamModel paramModel = new Enq940ParamModel();
    ...
    private ActionForward __setCommitDsp(ActionMapping map,
        ...
        // OKボタンクリック時遷移先
        urlForward = map.findForward("enq940commit");
        cmn999Form.setUrlOK(urlForward.getPath());

        // メッセージのセット
        GsMessage gsMsg = new GsMessage();
        String textEnq = gsMsg.getMessage(req, "enq.enq900.07");
