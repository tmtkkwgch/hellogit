...
/**
 * <br>[機  能] スケジュール 個人設定 グループメンバー表示設定画面のアクションクラス
 * <br>[解  説]
 * <br>[備  考]
 *
 * @author JTS
 */
public class Sch093Action extends AbstractScheduleAction {

    /** Logging インスタンス */
    private static Log log__ = LogFactory.getLog(Sch093Action.class);
    ...
    public ActionForward executeAction(ActionMapping map, ActionForm form,
        ...
        Sch093Form schForm = (Sch093Form) form;
        if (form == null) {
        ...
        schForm.setSch093SortKeyLabel(sortLabel);
        //コマンドパラメータ取得
        String cmd = NullDefault.getString(req.getParameter("CMD"), "");
        if (cmd.equals("sch093kakunin")) {
            //確認
            forward = __doKakunin(map, schForm, req, res, con);
        } else if (cmd.equals("sch093commit")) {
            //登録 戻る
            forward = __doCommit(map, schForm, req, res, con);
        } else if (cmd.equals("sch093back")) {
            //戻る
            forward = __doBack(map, schForm, req, res, con);
        } else if (cmd.equals("sch093reload")) {
            forward = __doReload(map, schForm, req, res, con);
        } else {
            //デフォルト
            log__.debug("BB form = " + schForm);
    ...
    private ActionForward __doKakunin(
        ...
        Sch093Biz biz = new Sch093Biz(getRequestModel(req));
        boolean memberDspFlg = biz.isMemberDspConf(con);

        ActionErrors errors = form.validateCheck(con, umodel.getUsrsid(), req);
        if (!errors.isEmpty() && memberDspFlg == true) {
            addErrors(req, errors);
            Sch093ParamModel paramMdl = new Sch093ParamModel();
...
//        ActionForward urlForward = null;
        ...
        cmn999Form.addHiddenParam("sch093SortKey1", form.getSch093SortKey1());
        cmn999Form.addHiddenParam("sch093SortOrder1", form.getSch093SortOrder1());
        cmn999Form.addHiddenParam("sch093SortKey2", form.getSch093SortKey2());
        cmn999Form.addHiddenParam("sch093SortOrder2", form.getSch093SortOrder2());
        cmn999Form.addHiddenParam("sch093DefGroup", form.getSch093DefGroup());
        cmn999Form.addHiddenParam("sch093MemDspConfFlg", form.getSch093MemDspConfFlg());
    ...
    private void __setCompPageParam(
        ...
        cmn999Form.setIcon(Cmn999Form.ICON_INFO);
        cmn999Form.setWtarget(Cmn999Form.WTARGET_BODY);
        urlForward = map.findForward("sch093back");
     ...
     * @throws SQLException SQL実行時例外
     */
    private ActionForward __doBack(ActionMapping map, Sch093Form form,
            HttpServletRequest req, HttpServletResponse res, Connection con)
            throws SQLException {

        log__.debug("戻る");
        ActionForward forward = null;
        forward = map.findForward("sch093back");
        return forward;
    }
     ...
     * @throws SQLException SQL実行時例外
     */
    private ActionForward __doInit(ActionMapping map, Sch093Form form,
            HttpServletRequest req, HttpServletResponse res, Connection con)
            throws SQLException {
     ...
     * @throws SQLException SQL実行時例外
     */
    private ActionForward __doReload(ActionMapping map, Sch093Form form,
            HttpServletRequest req, HttpServletResponse res, Connection con)
            throws SQLException {
