...
/**
 * <br>[機  能] スケジュール 手動データ削除設定画面のアクションクラス
 * <br>[解  説]
 * <br>[備  考]
 *
 * @author JTS
 */
public class Sch083Action extends AbstractScheduleAdminAction {
    /** Logging インスタンス */
    private static Log log__ = LogFactory.getLog(Sch083Action.class);
    ...
    public ActionForward executeAction(ActionMapping map, ActionForm form,
        ...
        Sch083Form schForm = (Sch083Form) form;
     ...
     */
    private ActionForward __doDelete(ActionMapping map, Sch083Form form,
            HttpServletRequest req, HttpServletResponse res, Connection con)
            throws SQLException {
        ...
        try {
            ...
            Sch083Biz biz = new Sch083Biz(getRequestModel(req));
            Sch083ParamModel paramMdl = new Sch083ParamModel();
            paramMdl.setParam(form);
            biz.deleteSchedule(paramMdl, umodel, con);
            paramMdl.setFormData(form);

            commit = true;
        } catch (SQLException e) {
            log__.error("スケジュールの削除に失敗しました。" + e);
            throw e;
        } finally {
            if (commit) {
                con.commit();
        ...
        values.append("[year]" + form.getSch083DelYear());
        values.append("[month]" + form.getSch083DelMonth());
        schBiz.outPutLog(
                map, req, res,
     ...
     * @throws SQLException SQL実行時例外
     */
    private ActionForward __doKakunin(ActionMapping map, Sch083Form form,
            HttpServletRequest req, HttpServletResponse res, Connection con)
            throws SQLException {
    ...
    private void __setKakuninPageParam(
        ...
        //メッセージセット
        String msgState = "sakujo.kakunin.old.data.once";

        //年
        String textYear = gsMsg.getMessage(
                req, "cmn.year", String.valueOf(form.getSch083DelYear()));
        //ヶ月
        String textMonth = gsMsg.getMessage(
                req, "cmn.months", String.valueOf(form.getSch083DelMonth()));

        String nengetu = textYear + textMonth;

        cmn999Form.setMessage(msgRes.getMessage(msgState, nengetu));
        cmn999Form.addHiddenParam("backScreen", form.getBackScreen());
        cmn999Form.addHiddenParam("sch083DelYear", form.getSch083DelYear());
        cmn999Form.addHiddenParam("sch083DelMonth", form.getSch083DelMonth());
     ...
     * @throws SQLException SQL実行時例外
     */
    private ActionForward __doBack(ActionMapping map, Sch083Form form,
            HttpServletRequest req, HttpServletResponse res, Connection con)
            throws SQLException {
        log__.debug("戻る");
        return map.findForward("sch083back");
    }
     ...
     * @throws SQLException SQL実行時例外
     */
    private ActionForward __doInit(ActionMapping map, Sch083Form form,
            HttpServletRequest req, HttpServletResponse res, Connection con)
            throws SQLException {
