...
public class Sch040Form extends Sch100Form {
    ...
    public ActionErrors validateCheck(
        ...
        //編集モード且つ、編集画面表示モードが出欠回答者の場合、出欠回答の選択チェックのみ行う
        if (getCmd().equals(GSConstSchedule.CMD_EDIT)
                && String.valueOf(GSConstSchedule.EDIT_DSP_MODE_ANSWER)
                .equals(sch040EditDspMode__)) {

            if (sch040AttendAnsKbn__ == null
                   || !(String.valueOf(GSConstSchedule.ATTEND_ANS_NONE).equals(sch040AttendAnsKbn__)
                   || String.valueOf(GSConstSchedule.ATTEND_ANS_YES).equals(sch040AttendAnsKbn__)
                   || String.valueOf(GSConstSchedule.ATTEND_ANS_NO).equals(sch040AttendAnsKbn__))
                   ) {
