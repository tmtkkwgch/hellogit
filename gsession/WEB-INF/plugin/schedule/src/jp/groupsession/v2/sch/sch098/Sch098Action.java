...
/**
 * <br>[機  能] スケジュール グループスケジュール表示設定画面のアクションクラス
 * <br>[解  説]
 * <br>[備  考]
 *
 * @author JTS
 */
public class Sch098Action extends AbstractScheduleAction {

    /** Logging インスタンス */
    private static Log log__ = LogFactory.getLog(Sch098Action.class);
    ...
    public ActionForward executeAction(ActionMapping map, ActionForm form,
        ...
        Sch098Form schForm = (Sch098Form) form;
        if (form == null) {
        ...
        if (cmd.equals("sch098kakunin")) {
            //確認
            forward = __doKakunin(map, schForm, req, res, con);
        } else if (cmd.equals("sch098commit")) {
            //登録
            forward = __doCommit(map, schForm, req, res, con);
        } else if (cmd.equals("sch098back")) {
            //戻る
...
//        ActionForward urlForward = null;
        ...
        /** メッセージ スケジュール初期表示設定 **/
        String mkey1 = gsMsg.getMessage(req, "schedule.src.83")
        ...
        cmn999Form.addHiddenParam("sch098DefDsp", form.getSch098DefDsp());
     ...
     * @throws SQLException SQL実行時例外
     */
    private ActionForward __doCommit(ActionMapping map, Sch098Form form,
            HttpServletRequest req, HttpServletResponse res, Connection con)
            throws SQLException {
        log__.debug("登録");
        ActionForward forward = null;
        ...
        //DB更新
        Sch098Biz biz = new Sch098Biz(getRequestModel(req));
        RequestModel reqMdl = getRequestModel(req);
        BaseUserModel umodel = getSessionUserModel(req);

        Sch098ParamModel paramMdl = new Sch098ParamModel();
    ...
    private void __setCompPageParam(
        ...
        cmn999Form.setIcon(Cmn999Form.ICON_INFO);
        cmn999Form.setWtarget(Cmn999Form.WTARGET_BODY);
        urlForward = map.findForward("sch098back");
        cmn999Form.setUrlOK(urlForward.getPath());

        GsMessage gsMsg = new GsMessage();
        /** メッセージ スケジュール初期表示設定 **/
        String key1 = gsMsg.getMessage(req, "schedule.src.83")
        + gsMsg.getMessage(req, "cmn.display.settings");
        //メッセージセット
     ...
     * @throws SQLException SQL実行時例外
     */
    private ActionForward __doBack(ActionMapping map, Sch098Form form,
            HttpServletRequest req, HttpServletResponse res, Connection con)
            throws SQLException {

        log__.debug("戻る");
        ActionForward forward = null;
        forward = map.findForward("sch098back");
        return forward;
    }
     ...
     * @throws SQLException SQL実行時例外
     */
    private ActionForward __doInit(ActionMapping map, Sch098Form form,
            HttpServletRequest req, HttpServletResponse res, Connection con)
            throws SQLException {
