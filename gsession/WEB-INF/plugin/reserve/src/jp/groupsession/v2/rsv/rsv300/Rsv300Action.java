...
/**
 * <br>[機  能] 施設予約 ショートメール通知設定画面のアクションクラス
 * <br>[解  説]
 * <br>[備  考]
 *
 * @author JTS
 */
public class Rsv300Action extends AbstractReserveAction {

    /** Logging インスタンス */
    private static Log log__ = LogFactory.getLog(Rsv300Action.class);
     ...
     */
     public ActionForward executeAction(ActionMapping map,
         ...
         if (!rsvBiz.canSetSmailConf(con, getSessionUserSid(req))) {
             return getAuthErrorPage(map, req);
         }

         ActionForward forward = null;
         Rsv300Form rsvform = (Rsv300Form) form;
     ...
     private ActionForward __doInit(ActionMapping map,
         ...
         Rsv300Biz biz = new Rsv300Biz(getRequestModel(req), con);
      ...
      *
      */
     private boolean __canUseSmlConf(Rsv300Form form, HttpServletRequest req, Connection con)
     throws SQLException {
         RsvCommonBiz rsvBiz = new RsvCommonBiz();
         RsvAdmConfModel mdl = rsvBiz.getRsvAdminData(con, getSessionUserSid(req));
         if (mdl.getRacSmailSend() == GSConstReserve.RAC_SMAIL_SEND_KBN_ADMIN) {
             return false;
         }
