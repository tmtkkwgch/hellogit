...
public class Cir010Form extends AbstractGsForm {
    ...
    public ActionErrors validateCheck(RequestModel reqMdl) {
        ...
        String textDeleteCir = gsMsg.getMessage("cir.35");

        //削除回覧板SID
        GSValidateCircular.validateDeleteCir(
                errors, cir010delInfSid__, textDeleteCir);

        return errors;
    }

    /**
     * <br>[機  能] 入力チェックを行う(元に戻す)
     * <br>[解  説]
     * <br>[備  考]
     * @param reqMdl リクエスト情報
     * @return エラー
     */
    public ActionErrors validateComeBackCheck(RequestModel reqMdl) {
        ActionErrors errors = new ActionErrors();

        GsMessage gsMsg = new GsMessage(reqMdl);
        String textComeBackCir = gsMsg.getMessage("cir.36");

        //削除回覧板SID
        GSValidateCircular.validateDeleteCir(
                errors, cir010delInfSid__, textComeBackCir);
