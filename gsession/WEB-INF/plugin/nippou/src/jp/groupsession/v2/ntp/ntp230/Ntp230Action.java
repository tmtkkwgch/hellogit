...
/**
 * <br>[機  能] 日報 目標一覧画面のアクションクラス
 * <br>[解  説]
 * <br>[備  考]
 *
 * @author JTS
 */
public class Ntp230Action extends AbstractNippouAdminAction {

    /** Logging インスタンス */
    private static Log log__ = LogFactory.getLog(Ntp230Action.class);
    ...
    public ActionForward executeAction(ActionMapping map,
        ...
        Ntp230Form thisForm = (Ntp230Form) form;

        if (cmd.equals("backNtp230")) {
            forward = map.findForward("ntp120");
        } else if (cmd.equals("add") || cmd.equals("edit")) {
            forward = map.findForward("ntp231");
        } else if (cmd.equals("upTargetData")) {
            //上へボタンクリック
            forward = __doSortChange(map, thisForm, req, res, con, GSConstNippou.SORT_UP);
        } else if (cmd.equals("downTargetData")) {
            //下へボタンクリック
    ...
    private ActionForward __doInit(ActionMapping map,
        ...
        Ntp230Biz biz = new Ntp230Biz(getRequestModel(req));

        Ntp230ParamModel paramMdl = new Ntp230ParamModel();
