...
/**
 * <br>[機  能] 日報 スケジュール表示設定画面のアクションクラス
 * <br>[解  説]
 * <br>[備  考]
 *
 * @author JTS
 */
public class Ntp096Action extends AbstractNippouAction {

    /** Logging インスタンス */
    private static Log log__ = LogFactory.getLog(Ntp096Action.class);
    ...
    public ActionForward executeAction(ActionMapping map, ActionForm form,
        ...
        Ntp096Form ntpForm = (Ntp096Form) form;
        if (form == null) {
        ...
        if (cmd.equals("ntp096kakunin")) {
            //確認
            forward = __doKakunin(map, ntpForm, req, res, con);
        } else if (cmd.equals("ntp096commit")) {
            //登録
            forward = __doCommit(map, ntpForm, req, res, con);
        } else if (cmd.equals("ntp096back")) {
            //戻る
...
//        ActionForward urlForward = null;
        ...
        String mkey1 = "スケジュール表示設定";
        cmn999Form.setMessage(msgRes.getMessage(msgState, mkey1));

        cmn999Form.addHiddenParam("backScreen", form.getBackScreen());
        cmn999Form.addHiddenParam("ntp096SchKbn", form.getNtp096SchKbn());
        cmn999Form.addHiddenParam("ntp096InitFlg", form.getNtp096InitFlg());
     ...
     * @throws SQLException SQL実行時例外
     */
    private ActionForward __doCommit(ActionMapping map, Ntp096Form form,
            HttpServletRequest req, HttpServletResponse res, Connection con)
            throws SQLException {
        log__.debug("登録");
        ActionForward forward = null;
        ...
        //DB更新
        Ntp096Biz biz = new Ntp096Biz(con, getRequestModel(req));
        BaseUserModel umodel = getSessionUserModel(req);

        Ntp096ParamModel paramMdl = new Ntp096ParamModel();
    ...
    private void __setCompPageParam(
        ...
        cmn999Form.setIcon(Cmn999Form.ICON_INFO);
        cmn999Form.setWtarget(Cmn999Form.WTARGET_BODY);
        urlForward = map.findForward("ntp096back");
        cmn999Form.setUrlOK(urlForward.getPath());

        //メッセージセット
        String msgState = "touroku.kanryo.object";
        String key1 = "スケジュール表示設定";
     ...
     * @throws SQLException SQL実行時例外
     */
    private ActionForward __doBack(ActionMapping map, Ntp096Form form,
            HttpServletRequest req, HttpServletResponse res, Connection con)
            throws SQLException {

        log__.debug("戻る");
        ActionForward forward = null;
        forward = map.findForward("ntp096back");
        return forward;
    }
     ...
     * @throws SQLException SQL実行時例外
     */
    private ActionForward __doInit(ActionMapping map, Ntp096Form form,
            HttpServletRequest req, HttpServletResponse res, Connection con)
            throws SQLException {
