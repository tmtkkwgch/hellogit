...
/**
 * <br>[機  能] メイン 管理者設定 オペレーションログ検索画面のフォーム
 * <br>[解  説]
 * <br>[備  考]
 *
 * @author JTS
 */
public class Man250Form extends AbstractGsForm {
    ...
    /**
     * <br>[機  能] 検索条件パラメータをSAVEフィールドへ移行します。
     * <br>[解  説]
     * <br>[備  考]
     */
    public void saveSearchParm() {
        man250SvSltGroup__ = man250SltGroup__;
        man250SvSltUser__ = man250SltUser__;
        man250SvSltYearFr__ = man250SltYearFr__;
        man250SvSltMonthFr__ = man250SltMonthFr__;
        man250SvSltDayFr__ = man250SltDayFr__;
        man250SvSltHourFr__ = man250SltHourFr__;
        man250SvSltMinFr__ = man250SltMinFr__;
        man250SvSltYearTo__ = man250SltYearTo__;
        man250SvSltMonthTo__ = man250SltMonthTo__;
        man250SvSltDayTo__ = man250SltDayTo__;
        man250SvSltHourTo__ = man250SltHourTo__;
        man250SvSltMinTo__ = man250SltMinTo__;
        man250SvSltLogError__ = man250SltLogError__;
        man250SvSltLogWarn__ = man250SltLogWarn__;
        man250SvSltLogInfo__ = man250SltLogInfo__;
        man250SvSltLogTrace__ = man250SltLogTrace__;
        man250SvSltPlugin__ = man250SltPlugin__;
        man250SvOrderKey1__ = man250OrderKey1__;
        man250SvSortKey1__ = man250SortKey1__;
        man250SvOrderKey2__ = man250OrderKey2__;
        man250SvSortKey2__ = man250SortKey2__;
        man250SvKeyWord__ = man250KeyWord__;
        man250SvKeyWordKbn__ = man250KeyWordKbn__;
        man250SvSearchTarget__ = man250SearchTarget__;
    }
     ...
     * @return エラー
     */
    public ActionErrors validateMan250Check(RequestModel reqMdl) {
        ...
        int fYear = Integer.parseInt(man250SltYearFr__);
        int fMonth = Integer.parseInt(man250SltMonthFr__);
        int fDay = Integer.parseInt(man250SltDayFr__);

        UDate frDate = new UDate();
        frDate.setDate(fYear, fMonth, fDay);
        frDate.setZeroHhMmSs();

        if (frDate.getYear() != fYear
        || frDate.getMonth() != fMonth
        || frDate.getIntDay() != fDay) {
            msg = new ActionMessage("error.input.notfound.date",
                    gsMsg.getMessage("main.src.man250.31"));
            errors.add("error.input.notfound.date", msg);

        //開始時間
        } else if ((man250SltHourFr__.equals(String.valueOf(GSConstCommon.NUM_INIT))
                && !man250SltMinFr__.equals(String.valueOf(GSConstCommon.NUM_INIT)))
                || (!man250SltHourFr__.equals(String.valueOf(GSConstCommon.NUM_INIT))
                        && man250SltMinFr__.equals(String.valueOf(GSConstCommon.NUM_INIT)))) {

            msg = new ActionMessage("error.input.notvalidate.data",
                    gsMsg.getMessage("main.src.man250.32"));
        ...
        boolean toOk = false;
        int tYear = Integer.parseInt(man250SltYearTo__);
        int tMonth = Integer.parseInt(man250SltMonthTo__);
        int tDay = Integer.parseInt(man250SltDayTo__);

        UDate toDate = new UDate();
        toDate.setDate(tYear, tMonth, tDay);
        toDate.setZeroHhMmSs();

        if (toDate.getYear() != tYear
        || toDate.getMonth() != tMonth
        || toDate.getIntDay() != tDay) {
            msg = new ActionMessage("error.input.notfound.date",
                    gsMsg.getMessage("main.src.man250.33"));
            errors.add("error.input.notfound.date", msg);

        //終了時間
        } else if ((man250SltHourTo__.equals(String.valueOf(GSConstCommon.NUM_INIT))
                && !man250SltMinTo__.equals(String.valueOf(GSConstCommon.NUM_INIT)))
                || (!man250SltHourTo__.equals(String.valueOf(GSConstCommon.NUM_INIT))
                        && man250SltMinTo__.equals(String.valueOf(GSConstCommon.NUM_INIT)))) {

            msg = new ActionMessage("error.input.notvalidate.data",
                    gsMsg.getMessage("main.src.man250.34"));
            errors.add("error.input.notvalidate.data", msg);
        ...
        if (fromOk && toOk) {
            ...
            if (frDate.compareDateYMD(toDate) == UDate.SMALL) {
                msg = new ActionMessage("error.input.comp.text",
                        gsMsg.getMessage("main.src.man250.35"),
        ...
        //ログ出力対象
        if (!man250SltLogError__.equals("1")
                && !man250SltLogWarn__.equals("1")
                && !man250SltLogInfo__.equals("1")
                && !man250SltLogTrace__.equals("1"))  {
            msg = new ActionMessage("error.select.required.text",
                    gsMsg.getMessage("cmn.loglevel"));
            errors.add("error.select.required.text", msg);
        }

        //キーワード
        String textKeyWord = gsMsg.getMessage("cmn.keyword");
        if (!StringUtil.isNullZeroString(man250KeyWord__)) {
            //検索キーワード 最大文字数チェック
            boolean maxLengthFlg = __checkRange(
                    errors,
                    man250KeyWord__,
                    "man250KeyWord",
                    textKeyWord,
                    GSConstMain.MAX_KEYWORD_LENGTH_TITLE);

            if (maxLengthFlg) {
                //先頭スペースチェック
                if (ValidateUtil.isSpaceStart(man250KeyWord__)) {
                    msg = new ActionMessage("error.input.spase.start", textKeyWord);
                    errors.add("error.input.spase.start", msg);
                }
                //スペースのみチェック
                if (ValidateUtil.isSpace(man250KeyWord__)) {
                    msg = new ActionMessage("error.input.spase.only", textKeyWord);
                    errors.add("error.input.spase.only", msg);
                }
                //タブ文字チェック
                if (ValidateUtil.isTab(man250KeyWord__)) {
                    msg = new ActionMessage("error.input.tab.text", textKeyWord);
                    errors.add("error.input.tab.text", msg);
                }
            }
        }

        //検索対象入力チェック
        __validateSearchTarget(errors, man250KeyWord__, man250SearchTarget__, gsMsg);

        //ソート順
        if (man250SortKey1__ == man250SortKey2__) {
            msg = new ActionMessage("error.select.dup.list",
                                    gsMsg.getMessage(GSConstMain.TEXT_SORT));
    ...
    /**
     * <br>[機  能] 入力値の最大文字数をチェックします。
     * <br>[解  説]
     * <br>[備  考]
     * @param errors アクションエラー
     * @param value 項目の値
     * @param element 項目名
     * @param elementName 項目名
     * @param range 桁数
     * @return true: エラーなし false: エラーあり
     */
    private boolean __checkRange(
            ActionErrors errors,
            String value,
            String element,
            String elementName,
        ...
        if (value.length() > range) {
            msg = new ActionMessage("error.input.length.text", elementName, String.valueOf(range));
     ...
     */
    private boolean __validateSearchTarget(
            ActionErrors errors,
            String keyWord,
            String[] searchTarget,
            GsMessage gsMsg) {
