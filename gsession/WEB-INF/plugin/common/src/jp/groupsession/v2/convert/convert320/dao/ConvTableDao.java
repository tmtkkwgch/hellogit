package jp.groupsession.v2.convert.convert320.dao;
...
public class ConvTableDao extends AbstractDao {
    ...
    private List<SqlBuffer> __createSQL() throws SQLException {
        ...
        //WEBメール メール添付ファイルのバイナリーSID データ型を変更
        sql = new SqlBuffer();
        sql.addSql(" alter table WML_TEMPFILE alter column WTF_SID type bigint;");
        sqlList.add(sql);


        //オペレーションログINDEX
        sql = new SqlBuffer();
        sql.addSql(" create index CMN_LOG_INDEX1 on CMN_LOG(LOG_DATE, LOG_PLUGIN, LOG_LEVEL);");
        sqlList.add(sql);
