...
/**
 * <br>[機  能] 稟議内容確認画面のフォーム
 * <br>[解  説]
 * <br>[備  考]
 *
 * @author JTS
 */
public class Rng030Form extends Rng010Form {
    ...
    public ActionErrors validateCheck(HttpServletRequest req) {
        ...
        String msg = gsMsg.getMessage(req, "cmn.comment");

        //-- コメントチェック --
        //未入力の場合はチェックを行わない
        if (StringUtil.isNullZeroString(rng030Comment__)) {
            return errors;
        }
    ...
    /**
     * <br>[機  能] 入力チェック(再申請時)を行う
     * <br>[解  説]
     * <br>[備  考]
     * @param req リクエスト
     * @return エラー
     */
    public ActionErrors validateCheckApplicate(HttpServletRequest req) {
        ActionErrors errors = new ActionErrors();

        //-- タイトルチェック --
        GsMessage gsMsg = new GsMessage();
        String msg = gsMsg.getMessage(req, "cmn.title");
        String msg2 = gsMsg.getMessage(req, "cmn.content");
