...
import jp.groupsession.v2.sml.sml340kn.Sml340knDao;
import jp.groupsession.v2.sml.sml360.Sml360Biz;
...
/**
 * <br>[機  能] ショートメール 個人設定 フィルタ登録確認画面のビジネスロジッククラス
 * <br>[解  説]
 * <br>[備  考]
 *
 * @author JTS
 */
public class Sml360knBiz {

    /** Logging インスタンス */
    private static Log log__ = LogFactory.getLog(Sml360knBiz.class);
    /** DBコネクション */
     ...
     */
    public Sml360knBiz(Connection con) {
        con__ = con;
    }
    ...
    public void setInitData(Connection con, RequestModel reqMdl,
                            Sml360knParamModel paramMdl, int userSid) throws SQLException {
        //アカウント名取得
        ...
        if (paramMdl.getSml360filterType() == 0) {
            paramMdl.setSml360filterTypeView(gsMsg.getMessage(GSConstSmail.ALL_CONDITION));
        } else {
            paramMdl.setSml360filterTypeView(gsMsg.getMessage(GSConstSmail.ANY_CONDITION));
        }

        //条件１
        if (paramMdl.getSml360condition1() != null) {
            paramMdl.setSml360conditionType1View(
                    __getViewCondition(paramMdl.getSml360conditionType1(), gsMsg));

            paramMdl.setSml360conditionExs1View(
                    __getViewConditionExs(paramMdl.getSml360conditionExs1(), gsMsg));
        }

        //条件２
        if (paramMdl.getSml360condition2() != null) {
            paramMdl.setSml360conditionType2View(
                    __getViewCondition(paramMdl.getSml360conditionType2(), gsMsg));

            paramMdl.setSml360conditionExs2View(
                    __getViewConditionExs(paramMdl.getSml360conditionExs2(), gsMsg));
        }

        //条件３
        if (paramMdl.getSml360condition3() != null) {
            paramMdl.setSml360conditionType3View(
                    __getViewCondition(paramMdl.getSml360conditionType3(), gsMsg));

            paramMdl.setSml360conditionExs3View(
                    __getViewConditionExs(paramMdl.getSml360conditionExs3(), gsMsg));
        }

        //条件４
        if (paramMdl.getSml360condition4() != null) {
            paramMdl.setSml360conditionType4View(
                    __getViewCondition(paramMdl.getSml360conditionType4(), gsMsg));

            paramMdl.setSml360conditionExs4View(
                    __getViewConditionExs(paramMdl.getSml360conditionExs4(), gsMsg));
        }

        //条件５
        if (paramMdl.getSml360condition5() != null) {
            paramMdl.setSml360conditionType5View(
                    __getViewCondition(paramMdl.getSml360conditionType5(), gsMsg));

            paramMdl.setSml360conditionExs5View(
                    __getViewConditionExs(paramMdl.getSml360conditionExs5(), gsMsg));
        }

        //ラベル名設定
        if (!(paramMdl.getSml360actionLabel().equals("0"))) {
            int wlbSid = Integer.parseInt(paramMdl.getSml360actionLabelValue());
            SmlLabelDao labelDao = new SmlLabelDao(con);
            SmlLabelModel labelData = labelDao.select(wlbSid);
            paramMdl.setSml360LabelView(labelData.getSlbName());
        }

        //フィルタリング対象のメール一覧
        if (paramMdl.getSml360doFilter() == Sml360Form.SML360_DOFILTER_YES) {
            Sml360Biz biz160 = new Sml360Biz();
            biz160.setMailList(con, paramMdl, userSid);
        }
    }
    ...
    public void doAddEdit(
        ...
        try {
            ...
            if (paramMdl.getSml360doFilter() == Sml360knForm.SML360_DOFILTER_YES) {
...
//        __entryFwAddress(paramMdl, wfMdl.getSftSid());
     ...
     * @param userSid ユーザSID
     */
    private void __setFilterEntryMdl(SmlFilterModel sfMdl, Sml360knParamModel paramMdl,
                                                    int userSid) {
        sfMdl.setUsrSid(userSid);
        sfMdl.setSftName(paramMdl.getSml360FilterName());
        sfMdl.setSftType(GSConstSmail.SFT_TYPE_ONE);
        sfMdl.setSftCondition(paramMdl.getSml360filterType());
        sfMdl.setSftTempfile(Integer.parseInt(paramMdl.getSml360tempFile()));
        sfMdl.setSftActionLabel(Integer.parseInt(paramMdl.getSml360actionLabel()));
        sfMdl.setSftActionRead(Integer.parseInt(paramMdl.getSml360actionRead()));
        sfMdl.setSftActionDust(Integer.parseInt(paramMdl.getSml360actionDust()));

        //ラベルを設定する場合
        if (paramMdl.getSml360actionLabel().equals("1")) {
            sfMdl.setSlbSid(Integer.parseInt(paramMdl.getSml360actionLabelValue()));
        }
    }
    ...
    private List<SmlFilterConditionModel> __getFilterConditionList(
        Sml360knParamModel paramMdl, int sftSid) throws SQLException {

        List<SmlFilterConditionModel> conditionList = new ArrayList<SmlFilterConditionModel>();

        if (paramMdl.getSml360condition1().equals(String.valueOf(GSConstSmail.CONDITION_ONE))) {
        ...
        if (paramMdl.getSml360condition2().equals(String.valueOf(GSConstSmail.CONDITION_TWO))) {
            conditionList.add(__createFilterConditionModel(sftSid,
        ...
        if (paramMdl.getSml360condition3().equals(String.valueOf(GSConstSmail.CONDITION_THREE))) {
            conditionList.add(__createFilterConditionModel(sftSid,
        ...
        if (paramMdl.getSml360condition4().equals(String.valueOf(GSConstSmail.CONDITION_FOUR))) {
            conditionList.add(__createFilterConditionModel(sftSid,
        ...
        if (paramMdl.getSml360condition5().equals(String.valueOf(GSConstSmail.CONDITION_FIVE))) {
            conditionList.add(__createFilterConditionModel(sftSid,
